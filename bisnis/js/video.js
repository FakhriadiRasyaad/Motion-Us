// bisnis/js/video.js — NO TOKEN, brand-aware
// Video Booth: 3:2 Output + Mirror + Filters + ZIP/PDF + GIF
...
async function guardRoles() {
  ...
  if (!session) { location.replace(LOGIN); throw new Error('no-session'); }
  const { data: prof } = await window.sb
    .from('profiles')
    .select('role')
    .eq('id', session.user.id)
    .maybeSingle();
  if (!prof?.role || !ALLOWED_ROLES.includes(prof.role)) {
    location.replace(LOGIN); throw new Error('role-denied');
  }
}
...
(() => {
  ...
  // ==============================
  // Supabase helpers (opsional, upload frame)
  // ==============================
  ...
  async function uploadFrameToBucket(file) {
    try {
      ...
      if (!user) return null; // tak pakai upload kalau tidak login
  ...
  // ==============================
  // Ekstrak Frame 3:2 (mirror + filters baked + BRAND watermark)
  // ==============================
  async function fastExtract(blob, fps) {
    ...
    for (let i = 0; i < totalFrames; i++) {
      ...
      ctx.fillText(BRAND, FRAME_W / 2, FRAME_H - 8);
      ctx.restore();
  ...
  downloadZip.onclick = async () => {
    ...
    const zip = new JSZip();
    const folderName = getFileName(BRAND);
  ...
  downloadPdf.onclick = async () => {
    ...
    saveAs(pdfBlob, `${getFileName(BRAND)}.pdf`);
    showProgress(false);
  ...
  // ==============================
  (async () => {
    try { await guardRoles(); } catch { return; } // redirect kalau gagal
    await listCameras();
